...
const VTKLoader = () => {
  ...
  const handleFileUpload = async (event) => {
    try {
      ...
      // Parse the VTK file
      const dataModel = {};
      parseLegacyASCII(text, dataModel);

      // Store the parsed data
      setParsedData(dataModel);
      setError(null);

      console.log("Parsed VTK data:", dataModel);
    } catch (err) {
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto">
      <div className="mb-6">
        <h2 className="text-2xl font-bold mb-4">VTK File Loader</h2>
      ...
      {parsedData && (
        <div className="mt-6">
          <h3 className="text-lg font-semibold mb-3">Parsed Data Structure:</h3>
          <div className="bg-gray-50 p-4 rounded-lg overflow-auto max-h-96">
            <pre className="text-sm">
              {JSON.stringify(parsedData.dataset, null, 2)}
            </pre>
          </div>
        </div>
      )}
